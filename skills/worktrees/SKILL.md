---
name: worktrees
description: Git worktree 管理与并行开发工作流。当需要并行开发多个 feature、隔离实验性改动、同时处理多个 bug、保持工作目录干净、OMO（One Mind, One Task）工作流、子任务并行、分支管理、多工作区协作、CI 矩阵构建时使用。
---
...
关键词命中即触发：git worktree/并行开发/隔离/多分支同时/工作区/OMO/子任务并行。
