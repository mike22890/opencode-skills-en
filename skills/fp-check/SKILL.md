---
name: fp-check
description: 安全漏洞的验证与误报消除。当需要验证疑似安全漏洞、消除误报、确认 TRUE/FALSE POSITIVE、写 PoC、复现漏洞、评估漏洞严重性、CVSS 评分、安全报告撰写、bug bounty 验证、渗透测试验证时使用。
...
关键词命中即触发：漏洞验证/误报/TRUE/FALSE/PoC/复现/严重性/CVSS/bug bounty/渗透测试。
...
| 用户说 | 加载 |
|---|---|
| 标准验证/线性/单组件/simple/直接 | references/standard-verification.md |
| 深度验证/复杂/跨组件/race/deep | references/deep-verification.md |
| 门审查/gate/verdict/判决 | references/gate-reviews.md |
| 漏洞类/bug class/内存/注入/race/crypto | references/bug-class-verification.md |
| 误报模式/false positive/red flag/pattern | references/false-positive-patterns.md |
| 证据模板/文档/数据流/proof | references/evidence-templates.md |
