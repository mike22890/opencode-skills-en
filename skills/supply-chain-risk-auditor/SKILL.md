...
description: 软件供应链安全与依赖风险审计。当需要审计 npm/PyPI/Maven/Go 依赖、检测已知漏洞（CVE）、检查废弃包/未维护包、分析依赖树、检测恶意包、license 合规、SBOM 生成、供应链攻击面评估、依赖锁定文件审查时使用。
...
关键词命中即触发：依赖审计/CVE/漏洞/废弃包/恶意包/license/SBOM/供应链/锁定文件。
