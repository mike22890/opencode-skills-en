...
description: 安全视角的代码变更审查（diff review）。当需要审查 PR/MR、检查代码改动的安全性、评估变更的爆炸半径（blast radius）、检测引入的漏洞、安全 code review、审查敏感操作（权限、加密、认证）、CI 中的安全检查、上线前安全审计时使用。
...
关键词命中即触发：PR审查/MR审查/代码变更/安全审查/上线前/合并前/权限/加密/认证/敏感操作。
...
| 用户说 | 加载 |
|---|---|
| 方法论/流程/phase/步骤/工作流 | methodology.md |
| 对抗/攻击者/利用场景/建模/adversarial | adversarial.md |
| 报告/输出/格式/模板/write | reporting.md |
| 漏洞模式/回归/reentrancy/access/pattern | patterns.md |
