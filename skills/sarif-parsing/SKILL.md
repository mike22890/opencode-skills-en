---
name: sarif-parsing
description: SARIF 格式扫描结果的解析与处理。当需要处理 CodeQL/Semgrep/ESLint 等工具的扫描输出、聚合安全 findings、去重、过滤 false positive、生成安全报告、CI 结果解析、漏洞趋势分析、合规报告、安全数据可视化时使用。
...
关键词命中即触发：SARIF/扫描结果/解析/聚合/去重/误报过滤/安全报告/CI/漏洞趋势/合规。
...
| 用户说 | 加载 |
|---|---|
| jq/查询/过滤/快速/一行 | resources/jq-queries.md |
| Python/pysarif/sarif-tools/脚本/处理 | resources/sarif_helpers.py |
