---
name: semgrep
description: Semgrep 静态代码分析与安全扫描。当需要快速代码扫描、安全审计、bug 检测、代码规范 enforcement、自定义规则编写、CI 集成、多语言扫描、pattern matching、代码质量检查、开源漏洞检测、license 合规检查时使用。
...
关键词命中即触发：Semgrep/静态分析/扫描/安全审计/bug/规则/CI/多语言/pattern/漏洞检测/license。
...
| 用户说 | 加载 |
|---|---|
| 规则集/catalog/选择/available/rules | references/rulesets.md |
| 扫描模式/过滤/important/run-all/重要 | references/scan-modes.md |
| 工作流/执行步骤/扫描流程/step | workflows/scan-workflow.md |
