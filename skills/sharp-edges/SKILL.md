---
name: sharp-edges
description: 识别代码中危险的 API、配置和设计陷阱。当需要安全设计审查、查找 footgun API、检测不安全配置、识别反模式、安全架构评估、代码危险模式识别、预防性安全审查、开发者安全培训材料时使用。
...
关键词命中即触发：危险API/footgun/不安全配置/反模式/安全架构/代码危险/预防性审查/加密/认证/配置陷阱。
...
| 用户说 | 加载 |
|---|---|
| 加密/API/密码学/algorithm/cipher | references/crypto-apis.md |
| 配置/验证/constructor/参数/陷阱 | references/config-patterns.md |
| 认证/授权/session/auth/登录 | references/auth-patterns.md |
| 案例/真实/历史/OpenSSL/GMP/case | references/case-studies.md |
| 语言/C++/Go/Rust/Python/JS/footgun | references/lang-*.md |
