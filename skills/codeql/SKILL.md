---
name: codeql
description: 使用 CodeQL 进行代码安全扫描与漏洞检测。当需要 SAST 静态分析、代码安全审计、找漏洞、数据流分析、污点追踪、检测注入/XSS/SSRF/路径遍历等安全漏洞、CI 安全扫描、代码质量门禁、合规检查时使用。
...
关键词命中即触发：CodeQL/静态分析/SAST/漏洞扫描/数据流/污点追踪/注入/XSS/SSRF/路径遍历/CI扫描/合规。
...
| 用户说 | 加载 |
|---|---|
| 建数据库/构建/CodeQL database/编译/提取 | workflows/build-database.md |
| 数据扩展/extension/模型/API包装/custom | workflows/create-data-extensions.md |
| 跑分析/扫描/执行/query/规则集/run | workflows/run-analysis.md |
| macOS/Apple Silicon/arm64/编译失败/报错 | references/macos-arm64e-workaround.md |
| 构建失败/fix/修复/编译错误 | references/build-fixes.md |
| 质量评估/数据库质量/检查/file count | references/quality-assessment.md |
| 规则集/catalog/查询包/语言/available | references/ruleset-catalog.md |
